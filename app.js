...
renderReport = function(){
  ...
  const netClass = net>=0 ? "stat-emerald" : "stat-red";
  const deadStockValue = v15DeadStock(60).reduce((a,p)=>a+(Number(p.stock)||0)*(Number(p.costPrice)||0),0);
  return `<div class="biz-dash">
    <div class="biz-hero">
      <div class="biz-hero-text">
        <h1>Monthly Business Report</h1>
        <p>Revenue, COGS, expenses na true net profit.</p>
        <div class="biz-month-pill"><input type="month" id="reportMonthSelect" value="${escapeHtml(key)}"/></div>
      </div>
      <div class="biz-illustration">📋📈🥧</div>
    </div>
    <div class="stat-grid">
      <div class="stat-card stat-green"><div class="stat-icon">💵</div><div><div class="stat-label">Revenue</div><div class="stat-value">${fmt(revenue)}</div></div></div>
      <div class="stat-card stat-orange"><div class="stat-icon">📦</div><div><div class="stat-label">COGS</div><div class="stat-value">${fmt(cogs)}</div></div></div>
      <div class="stat-card stat-blue"><div class="stat-icon">📈</div><div><div class="stat-label">Gross Profit</div><div class="stat-value">${fmt(gross)}</div></div></div>
      <div class="stat-card ${netClass}"><div class="stat-icon">💰</div><div><div class="stat-label">Net Profit</div><div class="stat-value">${fmt(net)}</div></div></div>
    </div>
    <div class="biz-panel">
      <h2>⭐ Top Products</h2>
      ${top.length?`<table><tbody>${top.map(([n,v])=>`<tr><td>${escapeHtml(n)}</td><td class="right mono">${fmt(v)}</td></tr>`).join("")}</tbody></table>`:`<div class="biz-empty"><p>Hakuna data.</p><div class="biz-empty-illustration">📦</div></div>`}
    </div>
    <div class="biz-panel">
      <h2>🏠 Inventory Position</h2>
      <div class="inv-grid">
        <div class="inv-item"><div class="stat-icon">📦</div><div><span>Stock Cost Value</span><b>${fmt(v15StockValue())}</b></div></div>
        <div class="inv-item"><div class="stat-icon">🛒</div><div><span>Potential Sales</span><b>${fmt(v15PotentialSalesValue())}</b></div></div>
        <div class="inv-item"><div class="stat-icon">📈</div><div><span>Potential Gross Profit</span><b>${fmt(v15PotentialSalesValue()-v15StockValue())}</b></div></div>
        <div class="inv-item"><div class="stat-icon">📭</div><div><span>Dead Stock Value</span><b>${fmt(deadStockValue)}</b></div></div>
      </div>
    </div>
  </div>`;
}
